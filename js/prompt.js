...
function buildReviewPrompt(guideline, questionText, devilsAdvocate) {
  ...
  if (devilsAdvocate) {
    ...
    lines.push("지적하는 각 항목은 반드시 다음 세 가지를 명시적으로 구분하여 제시하십시오:");
    lines.push("  (a) 트집의 논리: 어떤 근거로 이의를 제기할 수 있는가");
    lines.push("  (b) 출제자가 방어 가능한지: 출제자가 이 이의를 반박·방어할 수 있는가");
    lines.push("  (c) 억지인지 진짜 문제인지: 단순한 억지·트집인지, 아니면 실제로 수정이 필요한 진짜 오류인지");
  ...
  if (devilsAdvocate) {
    lines.push('판정 바로 다음 줄(응답 첫머리)에 다음 안내 문구를 그대로 적으십시오: "아래는 일부러 트집을 잡은 것으로, 대부분은 방어 가능한 억지일 수 있습니다."');
    lines.push("그 다음, 지적하는 각 항목을 위 (a)/(b)/(c) 세 구분으로 나누어 작성하십시오.");
    lines.push("사용자가 트집과 진짜 오류를 혼동하지 않도록, (c)에서 '진짜 문제'로 분류한 항목만 실제 수정이 필요함을 분명히 구분하십시오.");
  } else {
